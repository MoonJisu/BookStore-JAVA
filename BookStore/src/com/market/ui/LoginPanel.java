	...
	public LoginPanel(MainFrame main) {
		...
		signInBtn.addActionListener(e -> {
			...
			try {
				...
				// 3) DB 조회 또는 신규등록 (Welcome 내부에서 currentUserId 설정)
				Welcome.loginOrRegisterUser(user);

                // 4) ID가 유효한지 확인 후 화면 전환 (GUI 피드백 추가)
                if (Welcome.currentUserId > 0) {
                    JOptionPane.showMessageDialog(this, "로그인 성공! 환영합니다.", "로그인 성공", JOptionPane.INFORMATION_MESSAGE);
                    main.showPage("MENU");
                } else {
                    // DB 연결 실패나 ID 획득 실패 시 GUI 오류 메시지 출력
                    JOptionPane.showMessageDialog(this, 
                        "DB 연결 실패 또는 사용자 인증에 실패했습니다.\n(콘솔에서 자세한 오류를 확인하세요.)", 
                        "로그인 실패", 
